<?php
    ...
    $quarto = filter_input(INPUT_GET, 'mesa');
    ...
    if (mysqli_num_rows($resultDono)) {
        ...
        exit('false#Falha ao consultar o dono da mea.<br>'.var_dump($resultDono));
    }
    //var_dump($item);
...
<div class="row">
    <div class="col-md-24">
        <div>
            <h4>Mesa para transfer&ecirc;ncia deve estar <b>livre</b></h4>
        </div>
        <div class="form-group">
            <div class="col-md-12 text-left">
                <label for="quartoAtual" class="control-label">Mesa Atual:</label>
                <input type="text" name="quartoAtual" id="quartoAtual" class="form-control" readonly="YES" value="<?php echo $quarto?>">
            </div>
            <div class="col-md-12 text-left">
                <label for="quartoNovo" class="control-label">Mesa Novo:</label>
                ...
                <table class='table table-hover'>
                    ...
                    <?php 
                        ...
                        foreach ($itensNF as $itens) {
                            $nItens++;
                            echo "
                                <tr class='linha' id='linha_".$itens['id']."'>
                                    <td><input id='id_".$itens['id']."' class='chk' name='chk[]' value='".$itens['id']."' type='checkbox'></td>
    ...
    $(document).ready(function () {
        ...
        $('#RealizarTransferencia').click(function(){
            ...
            $("input[name='chk[]']:checked:enabled").each(function() {
                produtos=$(this).val()+","+produtos;
            });
            
            if (quartoNovo === '') {
                alert('Escolha a mesa!');
                $('#quartoNovo').focus();
                return;
            } else {
                $.ajax({
                    type: "POST",
                    data: 'qtoAtual=<?php echo $quarto;?>&qtoAntigo=<?php echo $dono['dono']?>&qtoNovo='+quartoNovo+'&qtItens='+<?php echo $nItens?>+'&prods='+produtos,
                    url: '../bar/banco.php?tipo=transferenciaQuarto',
                    success: function(m) {
                        //alert(m);
